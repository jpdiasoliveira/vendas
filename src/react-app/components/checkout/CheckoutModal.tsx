...
type PaymentNotice = { variant: "success" | "warning"; message: string };
    ...
    try {
      ...
      if (selectedMethod === "pix" && (pixBase64 || pixCopyPaste)) {
        ...
        setPaymentNotice({
          variant: "warning",
          message:
            "Não foi possível iniciar o pagamento com os dados recebidos. Tente novamente ou escolha outra forma.",
        });
      }
    } catch {
      ...
      <div className="fixed inset-0 z-[110] flex items-stretch justify-center p-0 sm:items-center sm:p-4">
        ...
        <div className="relative flex h-full max-h-[100dvh] w-full max-w-2xl flex-col overflow-hidden border border-white/50 bg-white/95 shadow-2xl backdrop-blur-xl sm:h-auto sm:max-h-[min(90dvh,720px)] sm:rounded-3xl">
          ...
          <div className="scrollbar-slim min-h-0 flex-1 overflow-y-auto overscroll-contain p-4 pb-2 pt-14 sm:p-8 sm:pt-10">
            <h2 className="mb-2 pr-10 font-playfair text-2xl font-bold text-[#1B4332] sm:text-3xl">
              Escolha a forma de pagamento
            </h2>
            <p className="mb-6 font-inter text-base text-[#5a4035]">
              Pedido #{orderId} • Total: R$ {total.toFixed(2)}
            </p>

            {error && (
              <div className="mb-4 rounded-xl bg-red-50 p-4 font-inter text-base text-red-600">{error}</div>
            )}

            {paymentNotice && (
              <div className={`mb-4 rounded-xl p-4 font-inter text-base ${noticeClass}`}>
              ...
              {paymentMethods.map((method) => {
                ...
                return (
                  <button
                    ...
                    className={`flex min-h-[72px] w-full items-center gap-4 rounded-2xl border-2 p-4 text-left transition-all duration-300 sm:p-6 ${
                      isSelected
                        ? "border-[#1B4332] bg-gradient-to-r from-[#1B4332]/5 to-[#FFD166]/5 shadow-lg"
                        : "border-[#1B4332]/10 bg-white/60 hover:border-[#1B4332]/30 active:scale-[0.99]"
                    }`}
                  >
                    <div
                      className={`shrink-0 rounded-xl bg-gradient-to-r p-3 text-white sm:p-4 ${method.color}`}
                    >
                      <Icon className="h-7 w-7 sm:h-8 sm:w-8" />
                    </div>
                    <div className="min-w-0 flex-1">
                      <h3 className="font-playfair text-lg font-bold text-[#1B4332] sm:text-xl">{method.name}</h3>
                      <p className="font-inter text-base text-[#5a4035]">{method.description}</p>
                    </div>
                    <div
                      className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full border-2 transition-all ${
                        isSelected ? "border-[#1B4332] bg-[#1B4332]" : "border-[#1B4332]/20"
                      }`}
                      aria-hidden
                    >
          ...
          <div className="shrink-0 border-t border-[#1B4332]/10 bg-white/95 px-4 py-4 pb-[max(1rem,env(safe-area-inset-bottom))] shadow-[0_-8px_24px_rgba(27,67,50,0.06)] sm:px-8">
            ...
            <p className="mt-3 text-center font-inter text-xs text-[#6D4C41]/80 sm:text-sm">
