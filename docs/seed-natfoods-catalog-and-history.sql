-- =============================================================================
-- Seed: catálogo Natfoods + pedidos de exemplo + auditoria (testes multi-tenant)
-- =============================================================================
-- Pré-requisitos:
--   1) Schema v1: docs/supabase-saas-multitenant-v1.sql
--   2) Loja + admin: docs/supabase-setup-admin-natfoods.sql (slug natfoods)
--
...
--
-- Idempotência: apaga pedidos demo (metadata.seed = 'natfoods_demo'); produtos
...
DECLARE
  ...
  -- Troque pela sua conta, ex.: 'suporte@ktech.mobi'
  v_admin_email text := '';
  v_store_id uuid;
  v_user_id uuid;
...
BEGIN
  IF trim(v_admin_email) = '' THEN
    RAISE EXCEPTION
      'Defina v_admin_email no DECLARE (aspas simples): ex. v_admin_email text := ''suporte@ktech.mobi''; — deve existir em auth.users.';
  END IF;

  SELECT id INTO v_store_id FROM public.stores WHERE slug = 'natfoods' LIMIT 1;
  IF v_store_id IS NULL THEN
    RAISE EXCEPTION 'Loja slug natfoods não encontrada. Execute docs/supabase-setup-admin-natfoods.sql antes.';
  END IF;
  ...
  INSERT INTO public.products (
    ...
    (
      ...
      'natfoods-banana-extra-picante-300g',
    ...
    (
      ...
      'natfoods-banana-castanha-para-300g',
    ...
    (
      ...
      'natfoods-banana-lime-cumin-300g',
    ...
    (
      ...
      'natfoods-banana-acai-guarana-300g',
    ...
    (
      ...
      'natfoods-banana-ervas-floresta-300g',
    ...
    (
      ...
      'natfoods-banana-defumado-mel-jatoba-300g',
  ...
  INSERT INTO public.orders (
    ...
    (
      ...
      jsonb_build_object('seed', 'natfoods_demo', 'nota', 'Pedido entregue — histórico completo'),
    ...
    (
      ...
      jsonb_build_object('seed', 'natfoods_demo'),
    ...
    (
      ...
      NULL, NULL,
      jsonb_build_object('seed', 'natfoods_demo', 'nota', 'Carrinho aguardando pagamento'),
  ...
  RAISE NOTICE 'Seed Natfoods OK — store_id=%, produtos=6, pedidos demo=4, audit_logs=3', v_store_id;
END;
$$;

SELECT name, price, stock, status
FROM public.products
WHERE store_id = (SELECT id FROM public.stores WHERE slug = 'natfoods' LIMIT 1)
ORDER BY name;

SELECT status, total, created_at
FROM public.orders
WHERE store_id = (SELECT id FROM public.stores WHERE slug = 'natfoods' LIMIT 1)
  AND (metadata->>'seed') = 'natfoods_demo'
ORDER BY created_at;
